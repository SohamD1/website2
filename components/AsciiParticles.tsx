...
const LOGO_TEXT = 'Ω'
const LOGO_Y = 0.72 // vertical center of the logo, as a fraction of height
const LOGO_ROWS = 18 // glyph height in cells — fixed, so it never rescales
...
type LogoCell = { r: number; c: number; ch: string; d: number }
...
export default function AsciiParticles() {
  ...
  useEffect(() => {
    ...
    // logo mask: text rendered at glyph-grid resolution, width scaled by
    // cos(t) to fake a spin about the Y axis
    const logo = document.createElement('canvas')
    const logoCtx = logo.getContext('2d', { willReadFrequently: true })
    if (!logoCtx) return
    let logoCols = 0
    let logoRows = 0
    let logoOffC = 0
    ...
    function setupLogo() {
      // fixed cell count, only shrinking if the window is too short for it
      logoRows = Math.max(1, Math.min(LOGO_ROWS, Math.floor(rows * 0.45)))
      logoCols = Math.max(1, Math.round(logoRows * LOGO_ASPECT))
      logo.width = Math.max(1, logoCols)
      logo.height = Math.max(1, logoRows)
      logoOffC = Math.floor((cols - logoCols) / 2)
      // centered in the bottom third, clamped to stay on screen
      logoOffR = Math.max(
        0,
        Math.min(rows - logoRows, Math.floor(rows * LOGO_Y - logoRows / 2))
      )
    }
    ...
    function drawLogoMask(t: number, out: LogoCell[]) {
      ...
      logoCtx!.clearRect(0, 0, logoCols, logoRows)
      logoCtx!.save()
      if (spin < 0) {
        logoCtx!.translate(logoCols, 0)
        logoCtx!.scale(-1, 1)
      }
      logoCtx!.font = `900 ${logoRows}px system-ui, sans-serif`
      logoCtx!.textBaseline = 'middle'
      logoCtx!.textAlign = 'center'
      logoCtx!.fillStyle = '#fff'
      logoCtx!.translate(logoCols / 2, logoRows / 2)
      // spin squashes the width; LOGO_STRETCH undoes the cell-grid stretch
      logoCtx!.scale(LOGO_STRETCH * (Math.abs(spin) || 0.001), 1)
      logoCtx!.fillText(LOGO_TEXT, 0, 2)
      logoCtx!.restore()

      const img = logoCtx!.getImageData(0, 0, logoCols, logoRows).data
      const bright = 0.4 + Math.abs(spin) * 0.6 // dim while edge-on
      for (let r = 0; r < logoRows; r++) {
        let firstCol = -1
        for (let c = 0; c < logoCols; c++) {
          const a = img[(r * logoCols + c) * 4 + 3]
          if (a < 20) continue
          ...
          out.push({
            r: gr,
            c: gc,
            ch: LOGO_TEXT[(c - firstCol) % LOGO_TEXT.length],
            d: (a / 255) * bright,
          })
        }
      }
